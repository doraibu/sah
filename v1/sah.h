...
struct sah_stack* sah_create(size_t);
void sah_destroy(struct sah_stack*);
...
static inline void sah_reset(struct sah_stack* s)
{
	s->sp = s->bp;
}
...
struct sah_stack* sah_create(size_t psize)
{
...
void sah_destroy(struct sah_stack* s)
{
	if (s == NULL) return;
